...
import { AddMovieForm } from "../components/media/forms/AddMediaForms"
import MovieCell from "../components/media/media-cell/MovieCell";
import { MediaStatusBtn } from "../components/btns/MediaStatusBtn";

// hooks
import { useAuth } from "../hooks/useFirebaseAuth";
import { useFetchAllMovieItems } from "../hooks/useFirestore";
...
const Movies = () => {
    ...
    const visibleMovies = filteredMovieItems.slice(0, visibleCount)
    ...
    return (
        <div className="flex flex-col items-center justify-start min-h-svh pb-20 bg-[#1a1a1a]">
            ...
            <div className="flex flex-col items-center justify-center w-full mt-6 px-3 gap-4 md:gap-0">
                ...
                { visibleCount < filteredMovieItems.length && (
                    <button
            ...
            {showAddMovieForm && user && (
                <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/80 overflow-y-auto p-4">
                    <div className="mt-10 mb-10 w-full sm:w-[80%] md:w-[50%]">
                        <AddMovieForm closeForm={() => setShowAddMovieForm(false)} />
                    </div>
                </div>
            )}
        </div>
